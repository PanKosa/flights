...
import data.wnioski as wnioski
...
app = dash.Dash(__name__, server=server, url_base_pathname='/flights/', csrf_protect=False)  # noqa: E501
...
airports = psql.read_sql("SELECT * FROM airports", conn)
...
corr_df = corr_df.drop('index', 0)

corr_df = corr_df.drop('index', 1)
...
names_a = []
for carrier in carriers:
    a = airports[airports["iata"] == carrier]
    a = list(a["name"])
    names_a.append(a[0])

air_names = pd.DataFrame({'IATA' : carriers, 'Full names' : names_a})
	...
	html.Div(
		...
		html.Div(
			...
			html.Div([
		        ...
		        html.Div(
		            [
				#html.P('Number of corelated plots:', id='corr_text',),  # noqa: E501
                                html.P(children = '', id='number_of_cor_plots_text'),
				dcc.Slider(
				    id='nn_slider',
				    min=1,
				    max=3,
		        ...
		        html.Div(
		            [
                                    ...
                                    dt.DataTable(
                                        ...
                                        filterable=False,
                                        sortable=True,
                                        selected_row_indices=["IATA", "Full names"],
                                        id='datatable'
                                    )
		            ],
		            className='six columns',
	...
	html.Div(
		...
		html.Div(
		    [
		        html.Div(
		            [
				html.P('Year:', id='year_map_text'),  # noqa: E501
				dcc.Slider(
				    id='year_map',
				    min=1987,
				    max=2008,
	...
	html.Div(
		...
		html.Div(
		    [
		        html.Div(
		            [
                                html.P('Min number of flights:', id='min_num_flights_text'), 
								dcc.Slider(
									id='numb_flights',
									min=1,
									max=100,
                                ...
                                html.P('Box Size:', id = 'box_size_text'), 
								dcc.Slider(
									id='box_size',
									min=1,
									max=200,
		    ...
		    html.Div(
		        ...
		        html.Div(
		                [
                            html.P('Data - Yaxis:'), # dodac ruchoma srednia a potem smotha dac -> czyli te biny z bokeh
					        dcc.Dropdown(
					            id='data_week',
...
def make_individual_figure(nn_slider, carrier_iata):
    ...
    layout_individual['yaxis'] = dict(title='Number of Delayed flights')
...
#datatable
@app.callback(Output('datatable', 'rows'), [Input('tabs','value')])
def update_datatable(x):
    return air_names.to_dict('records')

# number of coraleted 
@app.callback(Output('number_of_cor_plots_text', 'children'),
              [Input('tabs','value'), Input('nn_slider', 'value')])
def make_individual_figure(x, y):
    return 'Number of corelated plots: ' + str(y)
...
def make_individual_figure(x, year_map, iata_map):
    ...
    origin = airports[airports["iata"] == iata_map]    
...
#               opacity = float(number_of_flights[i])/float(number_of_flights.max()),
    ...
    layout_individual['title'] = 'Procent lotów spóźnionych z lotniska %s' % iata_map
...
# Year slider
@app.callback(Output('year_map_text', 'children'),
              [Input('tabs','value'), Input('year_map', 'value')])
def make_individual_figure(x, y):
    return 'Year: ' + str(y)
...
# min_num_flights_text
@app.callback(Output('min_num_flights_text', 'children'),
              [Input('tabs','value'), Input('numb_flights', 'value')])
def make_individual_figure(x, y):
    return 'Min number of flights: ' + str(y)

# box size
@app.callback(Output('box_size_text', 'children'),
              [Input('tabs','value'), Input('box_size', 'value')])
def make_individual_figure(x, y):
    return 'Box size: ' + str(y)
...
def make_delay_yearly(x, data_type_year, year_date):
    ...
    selected = flights_date[
        (flights_date.year >= year_date[0]) & (flights_date.year <= year_date[1])
    ]
...
# Year slider
@app.callback(Output('year_dep_txt', 'children'),
              [Input('tabs','value'), Input('year_data', 'value')])
def make_individual_figure(x, y):
    return 'Years: ' + str(y[0]) + ' - ' + str(y[1])
